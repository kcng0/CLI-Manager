...
- 文件浏览器新增可编辑路径栏和功能栏，支持上一级、项目根、刷新、新建文件/文件夹，并按本地、WSL、SSH 根路径解析输入；越出项目范围或包含 `..` 的路径会被拒绝。（Refs #226）
- 文件右键菜单补充剪切、创建副本、复制名称、在终端打开、属性；SSH 项目支持上传到当前目录和下载文件到本机。打开资源管理器仍仅限本地/WSL。（Refs #226）
- SSH 文件面板在远端 Agent 具备 `fileManage`（protocol `1.12`）时可写：新建、重命名、删除、复制/移动、保存文本与上传下载。Daemon 会拒绝旧 Agent 的写/上传/下载/属性请求并返回 `ssh_agent_capability_missing:fileManage`；前端此时只读并显示升级提示。上传下载按 512 KiB 分块，以适配 Agent 1 MiB 帧上限，单文件仍为 20 MiB。（Refs #226）
- 文件树点击会同步路径栏当前目录；SSH 根为 `/` 时地址不再变成 `//...`，也可解析粘贴的 POSIX 路径。写操作失败会弹出可理解的错误，上传撞名需确认覆盖。（Refs #226）
- SSH 主机编辑器新增端口转发分区，支持本地/远程/动态转发、启动时自动连接、启动/停止；交互式认证禁止后台隧道。应用延迟启动后会拉起标记了自动连接的转发。删除主机时后端会先停止该主机仍在运行的隧道；Windows 隧道进程挂到 Job Object。监听 `0.0.0.0` / `::` 会显示局域网暴露警告。端口转发列表在渲染期按主机过滤，避免 Zustand 5 selector 返回新数组导致打开主机编辑器时 React 无限更新。（Refs #226）
- SSH 远程目录选择器支持回到用户主目录、新建文件夹，以及删除空目录；删除前需确认，错误使用本地化文案。（Refs #226）
