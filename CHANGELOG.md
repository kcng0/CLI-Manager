...
- SSH 主机编辑器新增端口转发分区，支持本地/远程/动态转发、启动时自动连接、启动/停止；交互式认证禁止后台隧道。应用延迟启动后会拉起标记了自动连接的转发。删除主机时后端会先停止该主机仍在运行的隧道；Windows 隧道进程挂到 Job Object。监听 `0.0.0.0` / `::` 会显示局域网暴露警告。（Refs #226）
