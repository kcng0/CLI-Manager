...
- SSH 文件面板在远端 Agent 具备 `fileManage`（protocol `1.12`）时可写：新建、重命名、删除、复制/移动、保存文本与上传下载。旧 Agent 会提示升级，不再静默只读。（Refs #226）
- SSH 主机编辑器新增端口转发分区，支持本地/远程/动态转发、启动时自动连接、启动/停止；交互式认证禁止后台隧道。应用延迟启动后会拉起标记了自动连接的转发。（Refs #226）
- SSH 远程目录选择器支持回到用户主目录、新建文件夹，以及删除空目录。（Refs #226）
